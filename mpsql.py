...
class PsqlTable(object):
    ...
    def __inner_join(self, join):
        ...
        for _column_1, _column_2 in join_items:
            ...
            _join_str = "INNER JOIN {table} as {table_alias} ON {old_column} = {table_alias}.{join_column}".format(
                    table = join_table, table_alias = "t%s" % table_alias_counter, 
    ...
    def find(self, cond = None, column = None, join = None, **ext_kwargs):
        cond = cond or {}
        column = column or []
        ...
        sql_string += self.__inner_join(join)
        if cond:
    ...
    def __made_real(self, key, value, opera = 'eq'): # 产生关系表达式，像xx = yy或xx > yy这样的
        ...
        all_real_opera = {'eq': '=', 'in': 'IN', 'not': 'NOT', 'gt': '>', 'gte': '>=', 'ne': '!=', 
                'lt': '<', 'lte': '<='}
        opera = value and all_real_opera.get(opera.lower()) or 'is'
